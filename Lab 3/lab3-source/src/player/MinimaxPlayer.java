...
public class MinimaxPlayer implements PacManPlayer {
    ...
    public Move chooseMove(Game game) {
        ...
        for (Move m : legalMoves) {
            ...
            double score = getMinScore(next, m, 5, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY);
            /*
     ...
     */ 
    private double getMinScore(State s, Move lastPacmanMove, int depth, double alpha, double beta) {
        ...
        for (List<Move> move : ghostMoves) {
            ...
            double score = getMaxScore(next, lastPacmanMove, depth - 1, alpha, beta);
            res = Math.min(res, score);
            beta = Math.min(res, beta);
            if (beta <= alpha)
                break;                        
        }
        return res;
     ...
     */ 
    private double getMaxScore(State s, Move lastPacmanMove, int depth, double alpha, double beta) {
        if (depth == 0 || Game.isFinal(s))
            return evaluateState(s);
        ...
        for (Move move : pacmanMoves) {
            ...
            if (!move.equals(lastPacmanMove.getOpposite())) {
                State next = Game.getNextState(s, move);
                double score = getMinScore(next, move, depth - 1, alpha, beta);
